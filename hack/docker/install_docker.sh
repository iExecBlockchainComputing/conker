...
function install_docker() {
    ...
    systemctl status docker | grep Active | grep -q running && {
        docker_version=$( docker version | grep -A 10 Server | grep Version | awk '{print $2}')
    ...
    mkdir -p /usr/local/bin /etc/docker /opt/docker/down
    if [[ -f "/opt/docker/down/docker-${DOCKER_VER}.tgz" ]]; then
        ...
        if [[ -e /usr/bin/curl ]]; then
         ...
         mv ./docker-${DOCKER_VER}.tgz /opt/docker/down
    fi

    bash -c 'tar zxf /opt/docker/down/docker-*.tgz -C /opt/docker/down &&
    ...
    bash -c 'cat >/etc/systemd/system/docker.service <<EOF
[Unit]
        ...
        bash -c 'cat >/etc/docker/daemon.json <<EOF
{
  ...
  "data-root": "/workplace/encryptedData/docker"
}
EOF'
    else
...
#    systemctl daemon-reload && systemctl restart docker && sleep 8 && echo -e "${GREEN}[INFO] Docker installed successfully ${NC}"
}
